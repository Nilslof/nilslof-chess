    ...
    pub fn get_allowed_moves(&mut self, file: usize, rank: usize) -> Vec<(MoveType, usize, usize)> {
        ...
        match self.board[file][rank].unwrap().piece_type {
            PieceType::King => {
                for i in 0..3 {
                    for j in 0..3 {
                        ...
                        {
                            if self.board[file + 1 - i][rank + 1 - j].is_none()
                                || self.board[file + 1 - i][rank + 1 - j]
                                    .unwrap()
                                    .colour
                ...
                if file == 5 && rank == r {
                        ...
                        && !self.board[file][rank].unwrap().has_moved
                    {
                        ...
                        allowed_moves.push((MoveType::Castle, 2, r))
                    }
                        ...
                        && !self.board[file][rank].unwrap().has_moved
                    {
                        ...
                        allowed_moves.push((MoveType::Castle, 6, r))
                    }
                }
            }
            PieceType::Queen => {
                            ...
                            || (file_difference == 2 && rank_difference == 1)
                        {
                            if self.board[i][j].is_none()
                                || self.board[i][j]
                                    ...
                                    .equals(&self.board[file][rank].unwrap().colour)
                            {
                                if !self.is_in_check_after_move(MoveType::Normal, file, rank, i, j)
                                {
            ...
            PieceType::Pawn => {
                ...
                match colour {
                    Colour::White => {
                        if self.board[file][rank + 1].is_none() {
                            if self.board[file][rank + 1].is_none()
                                    ...
                                    .equals(&Colour::Black)
                            {
                                if !self.is_in_check_after_move(
                                    ...
                                    allowed_moves.push((MoveType::Normal, file, rank + 2));
                                }
                            }
                        }
                        if self.board[file][rank + 1].is_none()
                                ...
                                .equals(&Colour::Black)
                        {
                            if !self.is_in_check_after_move(
                                ...
                                allowed_moves.push((MoveType::Normal, file, rank + 1));
                            }
                        }

                        if self.board[file + 1][rank + 1].is_some() {
                            if !self.board[file + 1][rank + 1]
                                .unwrap()
                                .colour
                                .equals(&colour)
                            {
                                if !self.is_in_check_after_move(
                                    ...
                                    allowed_moves.push((MoveType::Normal, file + 1, rank + 1));
                                }
                            }
                        }
                        if file > 0 && self.board[file - 1][rank + 1].is_some() {
                            if !self.board[file - 1][rank + 1]
                                .unwrap()
                                .colour
                                .equals(&colour)
                    ...
                    Colour::Black => {
                        if self.board[file][rank - 1].is_some() {
                            if !self.board[file][rank - 1].unwrap().colour.equals(&colour) {
                                if !self.is_in_check_after_move(
                                    MoveType::Normal,
                                    file,
                                    rank,
                        ...
                        } else if rank == 7 && self.board[file][rank - 2].is_some() {
                            if !self.board[file][rank - 2].unwrap().colour.equals(&colour) {
                                if !self.is_in_check_after_move(
                                    MoveType::Normal,
                                    file,
                                    rank,
                        ...
                        if self.board[file - 11][rank - 1].is_some() {
                            if !self.board[file - 1][rank - 1]
                                .unwrap()
                                .colour
                                .equals(&colour)
                    ...
                    || (rank == 6 && colour.equals(&Colour::White))
                {
                    ...
                    if self.board[file][new_rank].is_none() {
                        if !self.is_in_check_after_move(
                            MoveType::Promotion(PieceType::Pawn),
                            file,
                            rank,
                            file,
                            new_rank,
                ...
                if self.possible_pessant.is_some() {
                    if self.possible_pessant.unwrap().1 == rank {
                        let new_file = self.possible_pessant.unwrap().0;
                        let mut file_difference: usize = if new_file > file {
                            new_file - file
                        } else {
                            file - new_file
                        };
                        if file_difference == 1 {
                            ...
                            if !self.is_in_check_after_move(
                                ...
                                allowed_moves.push((MoveType::Pessant, new_file, new_rank));
                            }
                        }
                    }
                }
